...
export default function Hero() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#0a0b2e] flex flex-col items-center">
      ...
      <div className="container mx-auto px-4 py-20 md:py-32 z-10 text-center">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-3 mb-8">
            {[
              { id: 'funnel', label: language === 'ar' ? 'القمع التسويقي' : 'Your Funnel' },
              { id: 'store', label: language === 'ar' ? 'المتجر' : 'Your Store' },
              { id: 'crm', label: language === 'ar' ? 'إدارة العملاء' : 'Your CRM' },
              { id: 'email', label: language === 'ar' ? 'التسويق بالبريد' : 'Email Marketing' },
              { id: 'courses', label: language === 'ar' ? 'الدورات التدريبية' : 'Online Courses' },
            ].map((tab) => (
              <button
                key={tab.id}
                ...
                className={`px-4 py-2 rounded-lg transition-colors ${
                  activeTab === tab.id
                    ? 'bg-[#4a5a8a] text-white'
                    : 'bg-transparent text-gray-300 hover:bg-[#4a5a8a]/30'
                }`}
              >
                {tab.label}
              </button>
