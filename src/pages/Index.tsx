  ...
  const steps = [
    {
      ...
      description: language === 'ar' 
        ? 'اختر من بين مجموعة متنوعة من القوالب المصممة مسبقاً لمختلف الأهداف.' 
        : 'Select from a variety of pre-designed templates for different goals.',
    },
    {
      number: "02",
      title: language === 'ar' ? 'خصص التصميم' : 'Customize Design',
      description: language === 'ar' 
        ? 'عدّل النصوص والألوان والصور لتناسب هويتك التجارية باستخدام أدواتنا سهلة الاستخدام.' 
        : 'Modify text, colors, and images to match your brand using our easy-to-use tools.',
    },
    {
      number: "03",
      title: language === 'ar' ? 'انشر صفحتك' : 'Publish Your Page',
      description: language === 'ar' 
        ? 'انشر صفحة الهبوط الخاصة بك بنقرة واحدة واحصل على رابط مخصص مباشرة.' 
        : 'Publish your landing page with one click and get a custom link instantly.',
    },
    {
      number: "04",
      title: language === 'ar' ? 'حلل النتائج' : 'Analyze Results',
      description: language === 'ar' 
        ? 'تتبع أداء صفحتك واحصل على رؤى قيمة لتحسين معدلات التحويل.' 
        : 'Track your page performance and get valuable insights to improve conversion rates.',
    },
  ];
          ...
          <div className="container mx-auto px-4 md:px-6">
            <div className="text-center mb-16">
              ...
              <p className="text-gray-600 text-lg max-w-2xl mx-auto">
                {language === 'ar' 
                  ? 'نجعل عملية إنشاء صفحات الهبوط سهلة وسريعة، حتى تتمكن من التركيز على أعمالك.' 
                  : 'We make the process of creating landing pages easy and fast, so you can focus on your business.'}
              </p>
            </div>
              ...
              {steps.map((step, index) => (
                <div key={index} className="flex flex-col items-center text-center group relative">
                  ...
                  {index < steps.length - 1 && (
                    <div className="hidden lg:block absolute top-8 left-[58%] w-[calc(100%-30px)] h-0.5 bg-gray-200">
        ...
        <section className="py-16 md:py-24 bg-white">
          <div className="container mx-auto px-4 md:px-6">
            <div className="text-center mx-auto max-w-3xl mb-16">
              ...
              <p className="text-gray-600 text-lg">
                {language === 'ar' 
                  ? 'إليك بعض الأسئلة الشائعة التي قد تساعدك' 
              ...
              {[
                {
                  ...
                  answer: language === 'ar' 
                    ? 'لا، منصتنا مصممة لتكون سهلة الاستخدام للجميع. يمكنك إنشاء صفحات هبوط احترافية باستخدام واجهة السحب والإفلات البديهية دون أي معرفة بالبرمجة.' 
                    : 'No, our platform is designed to be user-friendly for everyone. You can create professional landing pages using our intuitive drag-and-drop interface without any coding knowledge.'
                },
                {
                  question: language === 'ar' ? 'هل يمكنني استخدام نطاقي الخاص؟' : 'Can I use my own domain?',
                  answer: language === 'ar' 
                    ? 'نعم، يمكنك ربط صفحات الهبوط الخاصة بك بنطاقك الخاص. نحن نوفر خيارات لاستخدام نطاق فرعي مجاني أو ربط نطاقك المخصص.' 
                    : 'Yes, you can connect your landing pages to your own domain. We provide options to use a free subdomain or connect your custom domain.'
                },
                {
                  question: language === 'ar' ? 'هل أستطيع تصدير الكود الخاص بصفحتي؟' : 'Can I export the code of my page?',
                  answer: language === 'ar' 
                    ? 'نعم، توفر خططنا المتقدمة خيار تصدير كود HTML لصفحة الهبوط الخاصة بك، مما يتيح لك استضافتها على أي خادم تفضله.' 
                    : 'Yes, our advanced plans provide the option to export the HTML code of your landing page, allowing you to host it on any server you prefer.'
                },
                {
                  question: language === 'ar' ? 'هل يمكنني تجربة المنصة قبل الشراء؟' : 'Can I try the platform before purchasing?',
                  answer: language === 'ar' 
                    ? 'بالتأكيد! نحن نقدم خطة مجانية تتيح لك إنشاء صفحة هبوط واحدة واستكشاف ميزات المنصة. يمكنك الترقية في أي وقت إذا احتجت إلى ميزات أكثر.' 
                    : 'Absolutely! We offer a free plan that allows you to create one landing page and explore the platform features. You can upgrade anytime if you need more features.'
                },
                {
                  question: language === 'ar' ? 'هل صفحاتي متوافقة مع الأجهزة المحمولة؟' : 'Are my pages mobile-friendly?',
                  answer: language === 'ar' 
                    ? 'نعم، جميع صفحات الهبوط التي تنشئها على منصتنا متجاوبة تمامًا مع جميع أحجام الشاشات بشكل افتراضي، مما يضمن تجربة مثالية لزوارك على أي جهاز.' 
                    : 'Yes, all landing pages you create on our platform are fully responsive to all screen sizes by default, ensuring a perfect experience for your visitors on any device.'
                }
              ].map((faq, index) => (
